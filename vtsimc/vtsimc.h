        ...
        for(long ts = 1; ts < sts.length; ts++){
            ...
            if(c_idc.size() > 0){
                for(unsigned int i = 0; i < c_idc.size(); i++){
                    ...
                    if(sn[c_idc[i].beta[ts]] == 0)  sn[c_idc[i]].c[ts] += sn[c_idc[i]].m[ts] * sts.t_step / sn[c_idc[i]].v[ts]
                    else                            sn[c_idc[i]].c[ts] += sn[c_idc[i]].m[ts] / (sn[c_idc[i]].beta[ts] * sn[c_idc[i]].v[ts]) * (1 - exp(-sn[c_idc[i]].beta[ts] * sts.t_step));
                }
                for(unsigned int i = 0; i < vn.size(); i++){
