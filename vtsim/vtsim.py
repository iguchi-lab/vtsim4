...
def set_radiation(input):
    ...
    for r in input['room']:
        node = [n for n in input['sn'] if n.find(r) == 0 and '_is' in n]
...
def make_df(res, ix):
    ...
    if len(res[3]) != 0:    
        df_qv  = pd.DataFrame(np.array(res[3]).T,  index = ix, columns = get_keys(calc.v_net))
        dat_list.append({'fn': 'vent_qv.csv',  'title': '風量',  'unit': '[m3/s]', 'df': df_qv})
    else:
        df_qv = None

    if len(res[4]) != 0:    
        df_qt1 = pd.DataFrame(np.array(res[4]).T,  index = ix, columns = get_keys(calc.v_net))
        dat_list.append({'fn': 'thrm_qt1.csv', 'title': '熱量1', 'unit': '[W]', 'df': df_qt1})
    else:
        df_qt1 = None

    if len(res[5]) != 0:    
        df_qt2 = pd.DataFrame(np.array(res[5]).T,  index = ix, columns = get_keys(calc.t_net))
