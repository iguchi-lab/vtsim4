...
|設定項目|内容|設定値|省略時の設定|
|:--:|:--:|:--:|:--:|
|solve|連立方程式の解法を選択します。|LU分解法は 0 または vt.solve_LU、SOR法の場合は 1 または vt.solve_SORを設定|LU分解法|
|step_p|換気回路網計算時の偏微分で傾きを求める際の、圧力の微小変化量を設定します。|値|1e-6|
|vent_err|換気回路網計算時の許容残差を設定します。|値|1e-6|
|step_t|熱回路網計算時の偏微分で傾きを求める際の、温度の微小変化量を設定します。|値|1e-6|
|thrm_err|換気回路網計算時の許容残差を設定します。|値|1e-6|
|conv_err|換気回路網、熱回路網計算後の圧力と温度の収束判定に用いる許容変化量です。|値|1e-6|
|sor_ratio|SOR法で連立方程式を解く際の、緩和係数を設定します。|値|0.9|
|sor_err|SOR法で連立方程式を解く際の、収束判定に用いる許容残差です。|値|1e-6|
```
input = {
  'calc_sts': {
    'solve':  vt.soive_LU,
