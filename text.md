...
|step_p|換気回路網計算時の偏微分で傾きを求める際の、<br>圧力の微小変化量|値|1e-6|
|vent_err|換気回路網計算時の許容残差|値|1e-6|
|step_t|熱回路網計算時の偏微分で傾きを求める際の、<br>温度の微小変化量|値|1e-6|
|thrm_err|換気回路網計算時の許容残差|値|1e-6|
|conv_err|換気回路網、熱回路網計算後の<br>圧力と温度の収束判定に用いる許容変化量|値|1e-6|
|sor_ratio|SOR法で連立方程式を解く際の、緩和係数|値|0.9|
|sor_err|SOR法で連立方程式を解く際の、<br>収束判定に用いる許容残差|値|1e-6|
```
input = {
  'calc_sts': {
    'solve':  vt.soive_LU,
...
|p|圧力 [Pa]|値（リストでの設定も可）|0.0|
|t|温度 [℃]|値（リストでの設定も可）|20.0|
|c|濃度 [個/L]|値（リストでの設定も可）|0.0|
|isolation|日射量 [W/m<sup>2</sup>]|値（リストでの設定も可）|なし|
|h_input|発熱量 [W]|値（リストでの設定も可）|なし|
|v|気積 [m3<sup>3</sup>] （濃度計算用）|値（リストでの設定も可）|なし|
|m|粉塵発生量 [個/s]|値（リストでの設定も可）|なし|
|beta|沈着係数 [1/s]|値（リストでの設定も可）|なし|
...
vnで換気回路網の設定を行います。換気回路網は、「->」を挟んで始点と終点のノード名を記述（例えば「node1 -> node2」）したキー(key)とした辞書型(dict)で設定します。値(value)は数値（例えば、0.0や1.0など）もしくはリスト（[0.0, 1.0, ...]）で設定することができます。リストでの設定も可である値を数値で設定した場合、自動的にデータ長さ分のリストに変換され設定がされます。値(value)に記載された内容によって、自動的にどのようなネットワークが設定されたか判別されます。

### 共通
vnの設定では、共通して以下の項目の設定が必要です。
|設定項目|内容|値|省略時の設定|
|:--:|:--|:--|:--:|
|h1|始点の高さ [m]|値（リストでの設定も可）|0.0|
|h2|終点の高さ [m]|値（リストでの設定も可）|0.0|

### 単純開口 vt.VT_SIMPLE
alphaとareaが設定されている場合、単純開口の認識されます。
|設定項目|内容|値|
|:--:|:--|:--|
|alpha|流量計数 [-]|値（リストでの設定も可）|
|area|面積 [m<sup>2</sup>]|値（リストでの設定も可）|
